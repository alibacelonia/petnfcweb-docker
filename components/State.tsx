...
const StateSelect = ({ selectedState, handleChange }) => {
  ...
  useEffect(() => {
    ...
    const fetchPetType = async () => {
      try {
        const response = await fetch('/lottie/states.json'); // Replace with the actual API endpoint
        if (!response.ok) {
          throw new Error('Failed to fetch countries');
        }
        const responseData = await response.json();
        const filteredData = responseData.filter((state) => {return state.country_code === "AU"});
    ...
    <div className="sm:col-span-3">
        ...
        <div className="relative mt-1">
          ...
          <Transition
            ...
            afterLeave={() => setQuery('')}
          >
            <Combobox.Options className="absolute mt-1 z-50 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
